...
describe(aim2)
# highest is 22%, so will do 25 imputations

# do imputations
int_imp <- mice(aim2_int, m = 25,
                predictorMatrix = predmat,
                defaultMethod = c("pmm", "2l.bin", "polyreg", "polr"),
                where = wheremat_int,
                seed = 1111)
con_imp <- mice(aim2_con, m = 25,
                predictorMatrix = predmat,
                defaultMethod = c("pmm", "2l.bin", "polyreg", "polr"),
                where = wheremat_con,
                seed = 1111)
...
# 25 imputations
int_imp_cov <- mice(aim2_int_cov, m = 25,
                predictorMatrix = predmat,
                defaultMethod = c("pmm", "2l.bin", "polyreg", "polr"),
                where = wheremat_int_cov,
                seed = 1111)
con_imp_cov <- mice(aim2_con_cov, m = 25,
                predictorMatrix = predmat,
                defaultMethod = c("pmm", "2l.bin", "polyreg", "polr"),
...
cc_form <- formula(Y_raw ~ group.female + rcs(day.female, 3) + rcs(age.female, 3) + district.female + who_stage
           + edu_cat.female + edu_cat.male + rcs(soc_sup_ps.female, 3) + rcs(soc_sup_ns.female, 3))
lc_form <- formula(Y_lc ~ group.female + rcs(day.female, 3) + rcs(age.female, 3) + district.female + who_stage
           + edu_cat.female + edu_cat.male + rcs(soc_sup_ps.female, 3) + rcs(soc_sup_ns.female, 3))
wc_form <- formula(Y_wc ~ group.female + rcs(day.female, 3) + rcs(age.female, 3) + district.female + who_stage
           + edu_cat.female + edu_cat.male + rcs(soc_sup_ps.female, 3) + rcs(soc_sup_ns.female, 3))
```
...
# try it with ggeffects pool_predictions, requires using GEE
#gee_em <- complete(imp_prim_em, "all") %>%
#  map(~ geeglm(formula = Y_imp ~ group.female*tot.ss + rcs(day.female, 3) + rcs(age.female, 3) + district.female
#           + edu_cat.female + edu_cat.male + who_stage,
#           family = binomial,
#           id = clinic.female,
#           data = .x,
...
#pdf("fig14.pdf", height = 6, width = 10)
