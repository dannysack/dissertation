...
# pull in redcap data
raw <- read_csv("../Data/redcap_20211013.csv", 
...
df_female <- raw %>% select(record_id, data_de_inscricao, 
                            ...
                            estigma_1:paciente_9, codigo) %>% mutate(sex = "Female")
...
df_male <- raw %>% select(record_id, data_de_inscricao,
                          ...
                          estigma_1_h_1:paciente_9_h_1, codigo_homem) %>% mutate(sex = "Male")
...
redcap_cleaned <- df %>%
  select(record_id:pt_id, codigo, district,
         ...
         stigma_com:phq9, study_exc)

# now label columns
label(redcap_cleaned$record_id) <- "Record ID"
label(redcap_cleaned$reg_date) <- "Registration Date"
label(redcap_cleaned$pt_id) <- "Patient ID for OpenMRS Matching"
label(redcap_cleaned$codigo) <- "Patient ID for Drive Matching"
label(redcap_cleaned$group) <- "Study Group"
label(redcap_cleaned$age) <- "Age"
units(redcap_cleaned$age) <- "years"
label(redcap_cleaned$year) <- "Enrollment Year"
label(redcap_cleaned$day) <- "Enrollment Day"
label(redcap_cleaned$clinic) <- "Clinic"
label(redcap_cleaned$sex) <- "Sex"
label(redcap_cleaned$district) <- "District"
label(redcap_cleaned$rel_stat) <- "Relationship Status"
label(redcap_cleaned$edu_cat) <- "Education"
label(redcap_cleaned$job_cat) <- "Occupation"
label(redcap_cleaned$study_status) <- "Study Status"
label(redcap_cleaned$stigma_com) <- "Perceived Community Stigma"
label(redcap_cleaned$stigma_pt) <- "Patient Felt/Experienced Stigma"
label(redcap_cleaned$trust) <- "Physician Trust"
label(redcap_cleaned$cog_emp) <- "Cognitive Empathy"
label(redcap_cleaned$aff_emp) <- "Affective Empathy"
label(redcap_cleaned$soc_sup_ps) <- "Perceived Support"
label(redcap_cleaned$soc_sup_ns) <- "Needed Support"
label(redcap_cleaned$hivk) <- "HIV Knowledge (0-27)"
label(redcap_cleaned$phq9) <- "Patient Health Questionaire-9"
